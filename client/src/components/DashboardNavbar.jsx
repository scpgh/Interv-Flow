          ...
          >
            ...
            {tabs.map(({ key, label }) => (
              ...
              >
                {label}
                {key === 'community' && (
                  <span className="absolute -top-1 -right-2 px-1.5 py-0.5 text-[8px] bg-[#1E1B4B] text-[#818cf8] border border-[#818cf8]/40 font-bold rounded-full animate-bounce">
                    10k+
                  </span>
                )}
              </button>
        ...
        <div className="md:hidden border-t border-white/10 bg-[#09090b]/98 backdrop-blur-2xl px-6 py-4 flex flex-col gap-2">
          {tabs.map(({ key, label }) => (
            <button key={key} onClick={() => handleTabClick(key)} className={mTabCls(key)}>
              {label}
              {key === 'community' && (
                <span className="ml-2 px-1.5 py-0.5 text-[8px] bg-[#1E1B4B] text-[#818cf8] border border-[#818cf8]/40 font-bold rounded-full">10k+</span>
              )}
            </button>
          ))}
        </div>
